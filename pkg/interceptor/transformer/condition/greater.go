...
func (eq *Greater) Check(e api.Event) bool {
	fieldVal, err := eventops.GetNumber(e, eq.field)
	if err != nil {
		log.Warn("parse field %s value %v to number error: %v", eq.field, fieldVal, err)
		return false
	}

	return fieldVal.Greater(eq.value)
}
